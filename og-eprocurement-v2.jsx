...
const Dash = ({go}) => (
  <div className="space-y-5">
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <Stat icon={<I.Tender />} label="Active Tenders" value="14" change="3 new this week" up color="bg-orange-50 text-orange-600"/>
      ...
      <Stat icon={<I.Shield />} label="HSE Score" value="94.2%" change="+1.3%" up color="bg-violet-50 text-violet-600"/>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2 bg-white rounded-xl border border-stone-200/80 p-5">
        ...
        <div className="space-y-2">{TDR.slice(0,4).map(t=>(
      ...
      <div className="bg-white rounded-xl border border-stone-200/80 p-5">
        ...
        <div className="space-y-2">{HSE_INC.slice(0,3).map(h=>(
...
const TenderPg = () => { const [f,setF]=useState("All"); const ls=f==="All"?TDR:TDR.filter(t=>t.st===f); return (
  <div className="space-y-4">
...
const VendorPg = () => { const [sel,setSel]=useState(null); const CN={MY:"Malaysia",SG:"Singapore",NO:"Norway"}; return (
  <div className="space-y-4">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">{VND.map(v=>(
...
const HSEPg = () => (
  <div className="space-y-4">
    ...
    <tbody className="divide-y divide-stone-100">{HSE_INC.map(h=><tr key={h.id} className="hover:bg-stone-50"><td className="px-4 py-3 text-xs font-mono text-stone-400">{h.id}</td><td className="px-4 py-3 text-xs text-stone-600">{h.tp}</td><td className="px-4 py-3 text-xs text-stone-600 max-w-xs truncate">{h.d}</td><td className="px-4 py-3 text-xs text-stone-500">{h.site}</td><td className="px-4 py-3">{SB(h.sv)}</td><td className="px-4 py-3">{SB(h.st)}</td><td className="px-4 py-3 text-xs text-stone-400">{h.dt}</td></tr>)}</tbody></table></div>
  </div>
);
  ...
  <div className="space-y-4"><h2 className="text-lg font-bold text-stone-800">Spend Analytics</h2>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className="bg-white rounded-xl border border-stone-200/80 p-5"><h3 className="text-sm font-bold text-stone-700 mb-3">Vendor HSE Ranking</h3>
        <div className="space-y-2">{[...VND].sort((a,b)=>b.hse-a.hse).map((v,i)=>(
...
export default function App() {
  ...
  const pages={dashboard:<Dash go={setPg}/>,onboarding:<OnboardingPage />,tenders:<TenderPg />,vendors:<VendorPg />,hse:<HSEPg />,po:<POPg />,auction:<AuctionPg />,analytics:<AnalyticsPg />,ai:<AIPg />,settings:<SettingsPg />};
